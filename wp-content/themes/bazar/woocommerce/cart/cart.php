                        ...
                        <td class="product-thumbnail">
                            <?php
                                ...
                                if ( ! $_product->is_visible() || ( ! empty( $_product->variation_id ) && ! $_product->parent_is_visible() ) ) {
                                    ...
                                    printf('<a href="%s">%s</a>', esc_url( get_permalink( $cart_item['product_id'] ) ), $thumbnail );
                                }
                            ?>
                        </td>
                        ...
                        <td class="product-name">
                            <?php
                                if ( ! $_product->is_visible() || ( ! empty( $_product->variation_id ) && ! $_product->parent_is_visible() ) ) {
                                    ...
                                    printf('<a href="%s">%s</a>', esc_url( get_permalink( $cart_item['product_id'] ) ), apply_filters( 'woocommerce_cart_item_name', $_product->get_title(), $cart_item, $cart_item_key ) );
                                }
                        ...
                        <td class="product-quantity">
                            <?php
                                if ( $_product->is_sold_individually() ) {
                                    ...
                                    $product_quantity = sprintf( '<div class="quantity"><input type="number" name="cart[%s][qty]" step="%s" min="%s" max="%s" value="%s" size="4" title="' . _x( 'Qty', 'Product quantity input tooltip', 'yit' ) . '" class="input-text qty text" maxlength="12" /></div>', $cart_item_key, $step, $min, $max, esc_attr( $cart_item['quantity'] ) );
                                }
