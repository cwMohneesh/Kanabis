...
if ( ! function_exists( 'syn_woocommerce_loaded' ) ){
	...
	add_action( 'woocommerce_loaded', 'syn_woocommerce_loaded' );
}
	...
	function syn_meta_box_tracking() {
		...
		foreach( $tracking_number as $key => $track_number ){
			...
			if ( ! empty( $tracking_number[ $key ] ) && isset( $syn_carriers[ $tracking_carrier[ $key ] ] ) ){
				...
				echo( sprintf( '<a href="%s" target="_blank" class="tracking_link">' .__('Track this shipment from the carrier website', 'syn_shipping') . '</a>', $link ) );
			}
...
*/
	...
	function syn_woocommerce_admin_order_actions( $actions, $the_order ){
		...
		foreach( $tracking_number as $key => $tracking_num ){
			...
			$actions[ 'track_' . $key ] = array(
				...
				'image_url'	=> plugin_dir_url( dirname( dirname( __FILE__ ) ) . '/syn-shipping/assets/images/icons/track.png' ) . 'track.png'
			);
		}
	...
	function syn_update_tracking() {
		...
		foreach( $posts as $post ){
			...
			$date_shipped = get_post_meta( $post->post_id, '_date_shipped', true );
			...
			foreach( $tracking_numbers as $key => $tracking_number ){
				...
				if( ! isset( $syn_carriers[ $tracking_carriers[ $key ] ] ) ){
					$trackings[] = array();
					continue;
				}
