...
class SYN_Shipping_Method extends WC_Shipping_Method {
	...
	public function calculate_shipping( $package ){
		...
		foreach( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
			$hash .= $values['product_id'] . $values['quantity'];
		}
		
		$cart_hash = hash( 'MD5', $hash . $package[ 'destination' ][ 'country' ] . $package[ 'destination' ][ 'state' ] . $package[ 'destination' ][ 'postcode' ] );
		
		if ( !$this->debug )
			$cache_data = get_transient( 'shipping_' . $this->id . '_' . $cart_hash );
			
		if( empty( $cache_data ) ){
			
			$this->add_notice( sprintf( __( '%s: Shipping update required.', 'syn_shipping' ), $this->method_title ) );
			
			//Quote request
			if( $this->get_shipping_request( $package ) )
				set_transient( 'shipping_' . $this->id . '_' . $cart_hash, $this->rates );
			
		}else{
			
			$this->add_notice( sprintf( __( '%s: Shipping update not required.', 'syn_shipping' ), $this->method_title ) );
			$this->rates = $cache_data;
	...
	public function clear_transients() {
		...
		$wpdb->query( "DELETE FROM `$wpdb->options` WHERE `option_name` LIKE ('shipping_" . $this->id . "_%')" );
	}
